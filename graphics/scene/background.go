...
func NewBackground(renderer *render.Renderer, tex *texture.Manager) *Background {
	...
	geomSquarePlain := geometry.MakeSquareGeometry(geometry.CubeSideSimple) // -0.5..0.5 square
	return &Background{
...
func (m *Background) Render() {
	...
	m.Renderer().Orthogonal2D(1, 1) // x and y: -0.5..0.5

	m.matBack.Use()

	back := mgl32.Ident4()
	m.Renderer().Material(m.matBack)
	m.Renderer().Geometry(m.geomSquarePlain)
	m.Renderer().Render(&back)
